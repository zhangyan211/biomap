...
def get_gap_pic(pdb_path, pdb_name, gap_list, save_path, is_tool, is_gpcr):
    chain = pdb_name[-1]
    ...
    make_pdb_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/less30_pdb/'
    os.makedirs(make_pdb_path, exist_ok=True)
    make_pdb_path = os.path.join(make_pdb_path, pdb_name + '.pdb')
    ...
    is_tool_new = []
    is_gpcr_new = []
    is_tool_token= is_tool[0]
    is_gpcr_token = is_gpcr[0]
    if len(gap_list[2]) > 1:
        for i in range(len(gap_list[2])):
            if int(gap_list[2][i]) < 10:
                end_ind = int(gap_list[1][i])
                is_tool_token = is_tool_token or is_tool[i]
                is_gpcr_token = is_gpcr_token or is_gpcr[i]
            else:
                ...
                is_tool_token = is_tool[i]
                is_gpcr_new.append(is_gpcr_token)
                is_gpcr_token = is_gpcr[i]
    gap_list_new.append([start_ind, end_ind])
    is_tool_new.append(is_tool_token)
    is_gpcr_new.append(is_gpcr_token)
    ...
    residue_indexs = np.sort(list(smp_data.residue_index))
    for resident_index in residue_indexs:
        color_num = 2
        for ind, gap in enumerate(gap_list_new):
            if resident_index in range(gap[0],gap[1]+1):
                if is_tool_new[ind] == 1:
                    resident_indexs.append(0)
                elif is_gpcr_new[ind] == 1:
                    resident_indexs.append(1)
                else:
                    resident_indexs.append(color_num)
            if is_tool_new[ind] != 1 and is_gpcr_new[ind] != 1:
                color_num += 1
...
def process_epitope(np_exsamples, data_items):
    for row in data_items:
        ...
        # if True:
            ...
            # save_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/add_img'
            # save_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/test_img'
            save_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/gap_img'
            # save_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/less30_img'
            get_gap_pic(fpath, fname, [start, end, gap_len], save_path, is_tool, is_gpcr)
...
def start():
    ...
    pdb_map = pd.read_excel(excal_path, sheet_name=1)
    print(pdb_map)
    num_worker=100
