...
def get_gap_pic(pdb_path, pdb_name, gap_list, save_path, is_tool):
    chain = pdb_name[-1]
    ...
    make_pdb_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/gpcr_pdb/'
    ...
    if len(gap_list[2]) > 1:
        for i in range(len(gap_list[2])):
            if int(gap_list[2][i]) < 10:
                ...
                end_ind = int(gap_list[1][i])
    gap_list_new.append([start_ind, end_ind])
    # print(pdb_name, gap_list_new)
    # 提取氨基酸的b-factor值
    
    tool_start_ind = int(gap_list[0][is_tool])
    tool_end_ind = int(gap_list[1][is_tool])
    
    tool_color = -1
    for ind, gap_l in enumerate(gap_list_new):
        if tool_start_ind>= gap_l[0] and tool_end_ind <= gap_l[1]:
            tool_color = ind
    ...
    residue_indexs = list(smp_data.residue_index)
...
def process_epitope(np_exsamples, data_items):
    for row in data_items:
        ...
        # if True:
            ...
            # gpcr_data = pd.read_excel('/nfs_beijing/kubeflow-user/zhangyan_2023/code/protein_gap/1253_max_gap_over_30.xlsx')
            ...
            save_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/gpcr_img'
            
            get_gap_pic(fpath, fname, [start, end, gap_len], save_path, is_tool)
...
def start():
    ...
    csv_path = '/nfs_beijing/kubeflow-user/zhangyan_2023/code/py3Dmol/data/gpcr.csv'
